...
namespace Midjourney.Infrastructure.Handle
{
    ...
    public abstract class BotMessageHandler
    {
        ...
        protected void FindAndFinishImageTask(DiscordInstance instance, TaskAction action, string finalPrompt, SocketMessage message)
        {
            ...
            // 记录开始匹配任务
            Log.Information("BOT 开始匹配任务, MessageId: {MessageId}, Action: {Action}, FinalPrompt: {Prompt}, Hash: {Hash}", 
            ...
            if (task != null)
            {
                Log.Information("BOT 通过MessageId找到任务 (优先级1), TaskId: {TaskId}, MessageId: {MessageId}", 
                    task.Id, msgId);
            }
            ...
            Log.Information("BOT 成功匹配任务, TaskId: {TaskId}, MessageId: {MessageId}, Status: {Status}, Action: {Action}", 
